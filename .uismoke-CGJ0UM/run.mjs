...
// Svelte and the components expect these as globals.
for (const key of ['window','document','navigator','HTMLElement','Element','Node','CustomEvent','Event','getComputedStyle','requestAnimationFrame','cancelAnimationFrame','matchMedia','localStorage','MutationObserver','DOMParser','SVGElement']) {
...
const { mountOverlay, stores } = await import('/root/nagisa/.uismoke-CGJ0UM/overlay.mjs').then(m => globalThis.__nagisa ?? m);

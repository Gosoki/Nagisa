...
var NO_PATH = { path: null, dist: Infinity, s: 0 };
...
process.stdout.write(`pinholes (blocked cell, \u2265${PINHOLE_NEIGHBOURS}/8 neighbours open): ${pinholes.length}
`);
for (const [x, z, open] of pinholes.slice(0, 14)) {
  process.stdout.write(
    `  (${x.toFixed(0).padStart(5)}, ${z.toFixed(0).padStart(5)})  ${open}/8 open  slope ${(slopeAt(x, z) * 180 / Math.PI).toFixed(1)}\xB0  h ${heightAt(x, z).toFixed(2)}
`
  );
}
if (pinholes.length > 14) process.stdout.write(`  \u2026 and ${pinholes.length - 14} more
`);
process.stdout.write("\n");
var region = new Int32Array(size * size).fill(-1);
var spawn = SPAWN_POINTS2[0];
var start = toGrid(spawn.z) * size + toGrid(spawn.x);
var stack = [start];
region[start] = 0;
var mainRegion = 0;
while (stack.length) {
...
for (const lm of LANDMARKS2) {
  if (!reachable(lm.x, lm.z)) {
    problems++;
    process.stdout.write(`  UNREACHABLE mark   ${lm.kind} (${lm.x.toFixed(0)}, ${lm.z.toFixed(0)})
`);
  }
}
process.stdout.write(
...
process.stdout.write("\n");
var verdicts = [
  ["pinholes are rare", pinholes.length <= land * 4e-4, `${pinholes.length} over ${land} land cells`],

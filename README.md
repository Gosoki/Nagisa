...
| **Private islands** | Make your own island and get a five-letter code; the link is `?island=CODE`. Whoever made it is its keeper — admin there, and only there — each time they come back from the same browser — and can give it a name ("Design team's break room") that friends see in place of the code; someone they kick off it can't come back for half an hour. An invite link keeps working after everyone has left, and after a restart when `PERSIST_PATH` is set. |

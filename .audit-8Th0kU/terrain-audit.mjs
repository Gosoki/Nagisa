...
var ON_ROUTE_MARGIN = 4;
function onRoute(x, z) {
  ...
  for (const path of PATHS2) {
    ...
    for (let k = 1; k < pts.length; k++) {
      ...
      const dist = Math.hypot(x - (ax + vx * t), z - (az + vz * t));
      if (dist <= path.halfWidth + path.shoulder + ON_ROUTE_MARGIN) return true;
    }
  }
  return false;
}
var onRoutePinholes = pinholes.filter(([x, z]) => onRoute(x, z));
process.stdout.write(
  `pinholes (blocked cell, \u2265${PINHOLE_NEIGHBOURS}/8 neighbours open)
  total           ${pinholes.length}
  on a route      ${onRoutePinholes.length}   \u2190 the ones players meet
`
);
for (const [x, z, open] of (onRoutePinholes.length ? onRoutePinholes : pinholes).slice(0, 12)) {
  process.stdout.write(
...
process.stdout.write("\n");
var verdicts = [
  ["no pinholes where players walk", onRoutePinholes.length === 0, `${onRoutePinholes.length} on a route`],

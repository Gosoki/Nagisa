...
stores.daruma.set(race('walk', { startedAt: Date.now() - 800, endsAt: Date.now() + 4000, raceEndsAt: Date.now() + 170_000, racing: ['p1', 'p2'] }));
await settle(150);
...
check('and how far you have to go', text('DarumaHud').includes((shared.darumaCourseLength() + 1).toFixed(1) + ' m to the goal'), text('DarumaHud'));
...
await settle();
check('standing still, you are still racing', text('DarumaHud').includes('m to the goal') && !text('DarumaHud').includes('Seen moving'), text('DarumaHud'));
...
await settle();
check('home yourself: your place', text('DarumaHud').includes('Home — #2!') && text('DarumaHud').includes('Sawada (you)'), text('DarumaHud'));
...
await settle();
check('a quiz somewhere else, its card not up, moves nothing', !box('DarumaHud').querySelector('section')?.classList.contains('below'));
stores.quiz.set({ activity: 'q9', phase: 'question', round: 1, totalRounds: 8, questionId: Q.id, endsAt: Date.now() + 15_000, alive: ['p1'] });
await settle();
check('a quiz you are in keeps its slot; the race card goes under it', box('DarumaHud').querySelector('section')?.classList.contains('below'));
stores.quiz.set(null);
...
check('on a public shore it says nothing of coming back', !text('PlayerCard').includes('come back'));
let roomBeforeKick;
stores.room.subscribe((r) => (roomBeforeKick = r))();
stores.room.set({ id: 'isle-K7M2Q', name: 'x', population: 2, capacity: 20, kind: 'private', code: 'K7M2Q' });
await settle();
check('on a private island it says for how long they stay off', text('PlayerCard').includes('can’t come back for ' + shared.PROTOCOL.ISLAND_BAN_MIN + ' minutes'), text('PlayerCard'));
stores.room.set(roomBeforeKick);
await settle();
button('PlayerCard', /^Kick$/)?.click();
await settle();
